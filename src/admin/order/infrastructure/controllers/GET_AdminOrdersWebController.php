...
final class GET_AdminOrdersWebController extends Controller
{
    ...
    public function show(string $orderNumber): Response
    {
        ...
        $orderPayment = $this->orderPaymentRepository->findByOrderId($order->id());

        return Inertia::render('admin/orders/Show', [
            'payment' => $orderPayment ? [
                'paymentType' => $orderPayment->paymentType()->value,
                'paymentSummary' => $orderPayment->paymentSummary(),
                'status' => $orderPayment->status()->value,
                'mockTransactionId' => $orderPayment->mockTransactionId(),
                'amount' => $orderPayment->amount(),
                'processedAt' => $orderPayment->processedAt()->format('Y-m-d\TH:i:sP'),
            ] : null,
            'order' => [
                'id' => $order->id(),
